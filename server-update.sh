...
# ── Fix 6b: SiriusLabChatPanel onOpenProject — don't auto-navigate ────────────
python3 - artifacts/ai-chat/src/pages/star-lab.tsx << 'PYEOF'
import sys
path = sys.argv[1]
src = open(path).read()
old = """            onOpenProject={id => {
              loadProject(id);
              setNavMode("projects");
            }}
...
if old in src:
    open(path, 'w').write(src.replace(old, new, 1))
    print("✅ SiriusLabChatPanel onOpenProject no longer auto-navigates — explicit nav tag required")
else:
    print("ℹ️  Fix 6b already applied or pattern not found — skipping")
PYEOF

# ── Fix 9: CompleteAllModal — AbortController so X button truly cancels run ───
python3 - artifacts/ai-chat/src/pages/star-lab.tsx << 'PYEOF'
import sys
path = sys.argv[1]
src = open(path).read()
old_a = """  const base = getApiBase();
...
old_b = "  useEffect(() => { run(); }, []);"
new_b = "  useEffect(() => { run(); return () => { abortRef.current?.abort(); }; }, []);"
old_c = '            <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-lg transition-colors hover:bg-slate-100" title="Close">'
new_c = '            <button onClick={() => { abortRef.current?.abort(); onClose(); }} className="w-7 h-7 flex items-center justify-center rounded-lg transition-colors hover:bg-slate-100" title="Cancel and close">'
s = src
changed = False
...
if changed:
    open(path, 'w').write(s)
    print("✅ CompleteAllModal X button now properly cancels in-flight run via AbortController")
else:
