...
function Cell({ value, isSirius }: { value: boolean | string; isSirius?: boolean }) {
    ...
    return (
      <div className="flex justify-center">
        <div className="w-5 h-5 rounded-full flex items-center justify-center"
          style={{ background: isSirius ? "rgba(0,212,255,0.15)" : "rgba(255,255,255,0.06)" }}>
          <Check className="w-3 h-3" style={{ color: isSirius ? "#00D4FF" : "rgba(255,255,255,0.5)" }} />
        </div>
      </div>
    );
  }
  if (value === false) {
    return (
      <div className="flex justify-center">
        <X className="w-3.5 h-3.5" style={{ color: "rgba(255,255,255,0.15)" }} />
      </div>
    );
  }
  return (
    <div className="flex justify-center">
      <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.4)", fontSize: "10px" }}>
        {value}
      </span>
    </div>
  );
...
export function ComparePage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "hsl(226,45%,5%)", color: "white", fontFamily: "'SF Pro Display', 'Inter', system-ui, sans-serif" }}>
      ...
      <div className="border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        <div className="max-w-screen-xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href="/discover" className="text-sm text-white/40 hover:text-white/70 transition-colors">Discover</a>
      ...
      <div className="max-w-screen-xl mx-auto px-6 pt-16 pb-12 text-center">
        ...
        {/* Stats row */}
        <div className="inline-flex items-center gap-8 px-8 py-4 rounded-2xl" style={{ background: "hsl(226,45%,9%)", border: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="text-center">
            <div className="text-3xl font-bold" style={{ color: "#00D4FF" }}>{siriusFeatureCount}/{FEATURES.length}</div>
            <div className="text-white/40 text-xs mt-0.5">Features matched</div>
          </div>
          ...
          <div className="text-center">
            ...
            <div className="text-white/40 text-xs mt-0.5">Exclusive to Sirius</div>
          </div>
          <div className="w-px h-10" style={{ background: "rgba(255,255,255,0.08)" }} />
          <div className="text-center">
            <div className="text-3xl font-bold" style={{ color: "#A855F7" }}>0</div>
            <div className="text-white/40 text-xs mt-0.5">Competitors match all</div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-screen-xl mx-auto px-4 pb-20">
        <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.07)", background: "hsl(226,45%,7%)" }}>

          {/* Table header */}
          <div className="grid sticky top-0 z-10" style={{ gridTemplateColumns: "minmax(200px,1fr) repeat(7, 90px)", background: "hsl(226,45%,9%)", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="px-5 py-4">
              <span className="text-white/30 text-xs font-semibold uppercase tracking-wider">Feature</span>
            </div>
            {PLATFORMS.map(p => (
              <div key={p.name} className="py-4 px-1 flex flex-col items-center justify-center gap-1"
                style={{ background: p.isSirius ? "rgba(0,212,255,0.05)" : "transparent", borderLeft: p.isSirius ? "1px solid rgba(0,212,255,0.15)" : "1px solid rgba(255,255,255,0.04)" }}>
                {p.isSirius && <Star className="w-3 h-3" style={{ color: "#FFD700" }} />}
                <span className="text-xs font-bold leading-tight text-center" style={{ color: p.isSirius ? "#00D4FF" : "rgba(255,255,255,0.5)", fontSize: "10px" }}>
                  {p.name}
                </span>
              </div>
            ))}
            ...
            return (
              <div key={cat}>
                ...
                <button
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.35)" }}>{cat}</span>
                  {isCollapsed
                  ...
                  <div key={feature.name}
                    ...
                    <div className="px-5 py-3.5 flex flex-col gap-0.5">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium" style={{ color: feature.exclusive ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.65)" }}>
                          {feature.name}
                        </span>
                        {feature.exclusive && (
                      ...
                      <span className="text-xs" style={{ color: "rgba(255,255,255,0.28)" }}>{feature.desc}</span>
                    </div>
                    ...
                    {PLATFORMS.map(p => (
                      <div key={p.name} className="flex items-center justify-center py-3.5 px-1"
                        style={{
                          ...
                          borderLeft: p.isSirius ? "1px solid rgba(0,212,255,0.10)" : "1px solid rgba(255,255,255,0.03)"
                        }}>
        ...
        <div className="mt-16 text-center">
          <div className="inline-block px-10 py-10 rounded-3xl" style={{ background: "hsl(226,45%,9%)", border: "1px solid rgba(0,212,255,0.15)" }}>
            <div className="flex items-center justify-center gap-2 mb-3">
              <Star className="w-5 h-5" style={{ color: "#FFD700" }} />
              <span className="text-white font-bold text-xl">The only AI that works for your business</span>
              <Star className="w-5 h-5" style={{ color: "#FFD700" }} />
            </div>
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              <a href="/star-lab"
                className="px-7 py-3 rounded-xl font-bold text-sm transition-all hover:bg-white/10"
