...
function useSubscriptionContext() {
  ...
  const isPlus = ENTITLEMENT_PLUS in activeEntitlements;
  const isPro = ENTITLEMENT_PRO in activeEntitlements;
  const isSubscribed = isPlus || isPro;
  const tier: "free" | "plus" | "pro" = isPro ? "pro" : isPlus ? "plus" : "free";

  const currentOffering = offeringsQuery.data?.current;
  const plusPackage = currentOffering?.availablePackages.find(p => p.identifier === PACKAGE_PLUS);
  const proPackage = currentOffering?.availablePackages.find(p => p.identifier === PACKAGE_PRO);
